...
## Aktueller Stand
- Letzter abgeschlossener Fix: Bugfix Matchup-Pairings-Absturz (Session vom
  2026-08-12, vor M5) -- /matchups stuerzte auf Production ab, weil ein
  Freilos (Bye) player2 als fehlendes Feld (undefined) statt player2:null
  liefert. Details siehe Checkpoint-Log-Zeile unten
...
- NEU (M3): Limitless-Pairings-Response-Form (src/lib/limitless/types.ts,
  ...
  M2/M3. TEILWEISE AUFGELÖST seit 2026-08-12 (Debug-Session, Live-Fund auf
  Production): Nutzer meldete einen Absturz auf /matchups
  ("Matchup-Daten konnten nicht geladen werden: can't access property
  "deck", t.player2 is undefined"). Root Cause: Freilose (Byes) liefern
  player2 als FEHLENDES Feld (undefined), nicht als player2:null wie
  angenommen -- der Null-Check in aggregate.ts prüfte nur auf null. Fix:
  player2 in LimitlessPairing als optional typisiert, Check auf
  `pairing.player2 == null` erweitert (fängt null UND undefined). Die
  übrigen drei GATE-Punkte (Rundenabdeckung pro Call, exakte Feldnamen
  ausserhalb player2, outcome-Kodierung) bleiben unverifiziert.
